...
CI/local sin DSN el test simplemente se omite (no falla).
"""
...
pytestmark = pytest.mark.skipif(
    not _DSN, reason="define IDEAM_TEST_DSN (Postgres >=14) para ejercer el SQL real de IDF"
)
...
def conn():
    with psycopg.connect(_DSN, autocommit=True) as c:
        schema = f"idf_test_{uuid.uuid4().hex[:12]}"
        ...
        try:
            ...
            c.execute(_IDF_SQL.read_text(encoding="utf-8"))
            yield c
        finally:
            c.execute(f'DROP SCHEMA "{schema}" CASCADE')


def _sembrar(conn, codigo, sensores_vals, start="2020-06-01 00:00:00+00"):
...
def test_gana_el_sensor_mas_completo(conn):
    # 'A' tiene más slots (5) que 'B' (3); ninguno es 0257 -> gana A (su pico 9.0).
    codigo = "ESTMASCOMPLETO"
    _sembrar(conn, codigo, {
        "0240": [0.1, 0.1, 9.0, 0.1, 0.1],   # 5 slots, pico 9.0
        "0301": [0.1, 4.0, 0.1],             # 3 slots, pico 4.0
    })
    conn.execute("SELECT idf_compute_station(%s, 1)", (codigo,))
    assert _max_10min(conn, codigo) == pytest.approx(9.0, abs=0.05)


def test_0257_pierde_aunque_tenga_mas_slots(conn):
...
def test_empate_en_slots_desempata_por_lamina(conn):
    ...
    conn.execute("SELECT idf_compute_station(%s, 1)", (codigo,))
    assert _max_10min(conn, codigo) == pytest.approx(8.0, abs=0.05)


def test_pct_slots_reales_expuesto(conn):
    ...
    codigo = "ESTPCT"
    _sembrar(conn, codigo, {"0240": [0.1, 7.0, 0.1, 0.1]})
    conn.execute("SELECT idf_compute_station(%s, 1)", (codigo,))
    row = conn.execute(
    ...
    assert row is not None and row[0] == pytest.approx(100.0, abs=0.05)
